...
import PresentationComponent from "@/app/components/PresentationComponent";
...
const Page = () => {
  ...
  const chargerJSON = async () => {
    try {
      const response = await fetch("/robes-de-mariees-destockage.json");
      if (!response.ok) {
  ...
  const handleDressClick = (id) => {
    router.push(`/promo/robes-de-mariee-destockage/${id}`);
  };
      ...
      <div className="pt-20 bg-white">
        ...
        <div className="text-center py-16 px-5 bg-gradient-to-b from-[#FDE9E6] to-white">
          <PresentationComponent
            h1="Sélection des robes de mariée du style bohème-chic / champêtre en promotion
"
          />
        </div>
        {/* Section Filtres
